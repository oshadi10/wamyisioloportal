...
export function StudentPortal({ studentName, results, fees }: StudentPortalProps) {
  ...
  const myResults = results.filter((r) => r.student === studentName);
  const myFees = fees[studentName] || { total: 45000, paid: 30000 };
  const balance = myFees.total - myFees.paid;

  const downloadReport = () => {
    const filtered = results.filter((r) => r.student === studentName);
    const total = filtered.reduce((s, r) => s + Number(r.marks), 0);
    const mean = filtered.length ? (total / filtered.length).toFixed(1) : "0";
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <p className="text-xs text-muted-foreground">
              Select the term for your report form
            </p>
            ...
            <Button
              ...
              className="w-full bg-[#146f3a] hover:bg-[#0f5a2e]"
            >
